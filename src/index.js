...
async function bootstrapSkillsPage({ silent = false } = {}) {
  ...
  const starterSkills = [
    ...
    },
    {
      ...
      steps: [
        "Analyse the user's connected tool ecosystem and suggest cross-system workflow automations they haven't set up yet. Focus on combinations that chain tools from DIFFERENT sources (extension + MCP, Composio + extension, etc.).",
        "Triggers: \"suggest workflows\", \"what automations could I set up\", \"what skills am I missing\", \"recommend workflows\", \"workflow ideas\"",
        "Sources: cos_get_tool_ecosystem — complete tool, skill, and cron snapshot. cos_cron_list — current scheduled automations. [[Chief of Staff/Memory]] — user preferences and context.",
        "Instructions: Call cos_get_tool_ecosystem first to get the full ecosystem snapshot. Call cos_cron_list to see existing scheduled jobs. Review the user's memory for preferences, roles, and working patterns. Suggest 5–7 concrete workflows that combine tools from DIFFERENT sources. For each suggestion, provide: Name — short, descriptive. What it does — one sentence. Tools it chains — list the specific tool names from the ecosystem snapshot (do not invent tool names). Trigger — manual phrase, cron schedule, or reactive event. Why it's useful — reference the user's projects/context where possible. Prioritise cross-system combinations the user likely hasn't considered. Skip anything that duplicates an existing skill or scheduled job. Only reference tool names that appear in the ecosystem snapshot — never invent plausible-sounding tool names.",
        "Draft skill guidelines: see the Skill Designer skill for the full style guide, tool recommendations, and design process. When drafting from a suggestion, invoke Skill Designer rather than following these inline guidelines."
      ]
    },
    {
      name: "Skill Designer",
    ...
    },
    {
      ...
      steps: [
        ...
        "Acceptance: every guardrail in the audited skill appears in exactly one classification bucket. Every \"remove\" recommendation has a one-line justification. Every \"Keep (earned)\" item cites a specific evidence entry. The closing total Y equals the arithmetic sum of the per-item token estimates, and X equals the sum of the Remove items — state both sums explicitly so they can be checked. No live skill was modified.",
        "Rubric: report quotes guardrails verbatim. Recommendations use only remove / keep / annotate. Every Keep (earned) item carries an evidence citation. The closing total equals the sum of the itemised token estimates. Draft copy written only after explicit user confirmation.",
        "Output — chat panel only (no DNP write): one section per verdict — Remove (stale), Annotate (plausible-stale), Keep (earned), Keep (domain) — each item as: verbatim guardrail, ~tokens, one-line justification, evidence ref or \"no evidence found\". Then ADD THE NUMBERS UP: Y = sum of every per-item estimate across all four buckets; X = sum of the Remove items only. Close with: \"~X of this skill's ~Y tokens are removable (Z%). Want a (Draft) copy with removals applied?\" — where X and Y are those sums, not fresh estimates. A percentage that doesn't follow from the itemised numbers is a failed run.",
