...
function detectProvider(key) {
  ...
  // Generic sk- is ambiguous (OpenAI vs Moonshot/Kimi) — fall through to the
  // manual dropdown instead of guessing.
  return null;
}
...
const ONBOARDING_STEPS = [
  ...
  // ---- Step 2: API Key ----
  {
    ...
    render(ctx) {
      ...
      for (const opt of ["mistral", "anthropic", "openai", "gemini", "groq", "grok", "kimi"]) {
      ...
      const providerLabelsMap = { anthropic: "Anthropic (Claude)", openai: "OpenAI (GPT)", gemini: "Google (Gemini)", groq: "Groq (Llama)", grok: "Grok (xAI)", kimi: "Kimi (Moonshot)" };
      ...
      btnContainer.appendChild(createButtonGroup([
        {
          ...
          onClick: () => {
            ...
            const providerLabels = { anthropic: "Anthropic", openai: "OpenAI", gemini: "Gemini", mistral: "Mistral", groq: "Groq", grok: "Grok (xAI)", kimi: "Kimi (Moonshot)" };
            deps.iziToast.success({
              class: "cos-toast",
  ...
  // ---- Step 10: Local MCP Servers ----
  {
    ...
    render(ctx) {
      ...
      const connectedCount = Array.from(deps.getLocalMcpClients().values()).filter(e => e?.serverName).length;
