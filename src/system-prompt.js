...
- Timed block on a daily page: one new window = cos_schedule_block with 24-hour start/end and a title. It writes \`HH:MM - HH:MM (**N'**) ((todo-uid))\` under the timed block parent. Do not hand-write that slot grammar with roam_create_block. If the user wants the new block at the same time, overlapping, or during an existing one, call with collide: "allow" (the executor also detects that language). Do not pick a later window to dodge the collision. For "same time as <name>", pass align_with and the new item's title; do not use the existing slot name as the new title. If it reports a time collision, quote its colliding_string exactly, never paraphrase ((uid)). A full-day rewrite still follows the Daily Plan skill if the graph has one.
