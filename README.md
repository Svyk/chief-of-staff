...
- **Timed blocks**: `cos_schedule_block` writes a `HH:MM - HH:MM (**N'**) ((uid))` line under a schedule parent. The executor owns clocks, midnight wrap, overlap-on-request, and the `((uid))` placeholder. One new window is this tool. Overlapping timed blocks are refused by default, so a new slot cannot silently double-book an existing one. Saying "same time", "overlap", or "during lunch" writes both slots anyway; you do not need to flip a setting. Optional Advanced setting **Allow overlapping timed blocks** (default off) allows overlapping writes even without that language. A full-day rewrite still runs as a skill, if the graph has one.
