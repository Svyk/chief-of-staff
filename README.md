...
- **Agent max iterations** (Advanced, default 20) — cap on agent-loop iterations for normal chat. Raise for weaker models that spend one iteration per tool, or for long multi-write rearranges. Multi-write intents (rearrange / insert many seasons into a watch order) auto-boost to at least 32. Allowed range 10–40.
...
- **Agent iterations** — the reasoning loop is capped by **Agent max iterations** (default 20; range 10–40). Skills use **Skill max iterations** (default 16). Multi-write rearrange intents auto-boost the chat cap to at least 32.
