...
- **Timed blocks** — `cos_schedule_block` writes a `HH:MM - HH:MM (**N'**) ((uid))` line under a schedule parent. The executor owns clocks, midnight wrap, overlap, and the `((uid))` placeholder. One new window is this tool. A full-day rewrite still runs as a skill, if the graph has one.
