...
- **Timed blocks** — `cos_schedule_block` writes a `HH:MM - HH:MM (**N'**) ((uid))` line under a schedule parent. The executor owns clocks, midnight wrap, overlap-on-request, and the `((uid))` placeholder. One new window is this tool. Saying "same time", "overlap", or "during lunch" writes both slots without flipping the Advanced setting. A full-day rewrite still runs as a skill, if the graph has one.
...
- **Schedule parent** (Advanced, empty by default) — page title or block uid that owns new timed blocks. Empty uses today's daily page (an existing Nautilus Log child if present, otherwise a Schedule heading). Does not inject Nautilus onto a graph that never had it.
- **Sandbox schedule page** (Advanced, default `COS Daily Plan Sandbox`) — when the user message contains `[sandbox]`, timed blocks go under that page instead of today.
- **Allow overlapping timed blocks** (Advanced, default off) — when off, a new timed block that overlaps a different one is refused unless the user asks to overlap (same time, during, in parallel). When on, overlapping writes are allowed even without that language. Same task still reschedules in place. Saying "same time", "overlap", or "during lunch" writes both without flipping this setting.
